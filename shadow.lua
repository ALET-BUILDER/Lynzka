--[[
    ...
    ║   Speed Hack + Hologram + Aimbot + ESP  ║
    ╚══════════════════════════════════════════╝
]]
...
pcall(function() game:GetService("CoreGui"):FindFirstChild("ShadowHub"):Destroy() end)
pcall(function() LocalPlayer.PlayerGui:FindFirstChild("ShadowHub"):Destroy() end)
...
TBF.BackgroundColor3 = Color3.fromRGB(28, 28, 42)
...
CloseBtn.Text = "✕"; CloseBtn.Size = UDim2.new(0, 38, 0, 38)
CloseBtn.Position = UDim2.new(1, -38, 0, 0); CloseBtn.BackgroundTransparency = 1
CloseBtn.TextColor3 = Color3.fromRGB(255, 80, 80); CloseBtn.Font = Enum.Font.GothamBold
CloseBtn.TextSize = 18; CloseBtn.Parent = TitleBar

local MinBtn = Instance.new("TextButton")
MinBtn.Text = "—"; MinBtn.Size = UDim2.new(0, 38, 0, 38)
MinBtn.Position = UDim2.new(1, -76, 0, 0); MinBtn.BackgroundTransparency = 1
MinBtn.TextColor3 = Color3.fromRGB(255, 210, 60); MinBtn.Font = Enum.Font.GothamBold
MinBtn.TextSize = 18; MinBtn.Parent = TitleBar
...
TabPanel.BorderSizePixel = 0; TabPanel.Parent = MainFrame
Instance.new("UICorner", TabPanel).CornerRadius = UDim.new(0, 8)
local TL = Instance.new("UIListLayout", TabPanel)
TL.SortOrder = Enum.SortOrder.LayoutOrder; TL.Padding = UDim.new(0, 3)
local TP2 = Instance.new("UIPadding", TabPanel)
TP2.PaddingTop = UDim.new(0, 6); TP2.PaddingLeft = UDim.new(0, 5); TP2.PaddingRight = UDim.new(0, 5)
...
ContentPanel.BorderSizePixel = 0; ContentPanel.ClipsDescendants = true
...
do
    ...
    TitleBar.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
            dr = true; ds = i.Position; sp = MainFrame.Position end end)
    TitleBar.InputEnded:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then dr = false end end)
    UserInputService.InputChanged:Connect(function(i)
        if dr and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
            local d = i.Position - ds
            MainFrame.Position = UDim2.new(sp.X.Scale, sp.X.Offset + d.X, sp.Y.Scale, sp.Y.Offset + d.Y) end end)
end

local function createTab(name, icon, order)
    local btn = Instance.new("TextButton")
    btn.Name = name; btn.LayoutOrder = order or 0
    btn.Size = UDim2.new(1, 0, 0, 33); btn.BackgroundColor3 = Color3.fromRGB(32, 32, 48)
    btn.BorderSizePixel = 0; btn.Text = ""; btn.AutoButtonColor = false; btn.Parent = TabPanel
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 6)
    local bl = Instance.new("TextLabel", btn); bl.Text = icon .. "  " .. name
    bl.Size = UDim2.new(1, 0, 1, 0); bl.Position = UDim2.new(0, 10, 0, 0)
    bl.BackgroundTransparency = 1; bl.TextColor3 = Color3.fromRGB(175, 175, 200)
    bl.Font = Enum.Font.GothamSemibold; bl.TextSize = 13; bl.TextXAlignment = Enum.TextXAlignment.Left
    local pg = Instance.new("ScrollingFrame"); pg.Name = name .. "_Page"
    ...
    pg.Visible = false; pg.Parent = ContentPanel
    ...
    tabs[name] = {btn = btn, label = bl}; pages[name] = pg
    btn.MouseButton1Click:Connect(function()
        for n, t in pairs(tabs) do t.btn.BackgroundColor3 = Color3.fromRGB(32, 32, 48)
            t.label.TextColor3 = Color3.fromRGB(175, 175, 200); pages[n].Visible = false end
        btn.BackgroundColor3 = Color3.fromRGB(90, 45, 210)
        bl.TextColor3 = Color3.fromRGB(255, 255, 255); pg.Visible = true; currentTab = name end)
    return pg
end

local function createSection(p, text)
    local l = Instance.new("TextLabel"); l.Size = UDim2.new(1, -4, 0, 24)
    l.BackgroundColor3 = Color3.fromRGB(90, 45, 210); l.BackgroundTransparency = 0.75
    l.BorderSizePixel = 0; l.Text = "  " .. text; l.TextColor3 = Color3.fromRGB(190, 160, 255)
    l.Font = Enum.Font.GothamBold; l.TextSize = 12; l.TextXAlignment = Enum.TextXAlignment.Left
    l.Parent = p; Instance.new("UICorner", l).CornerRadius = UDim.new(0, 5) end

local function createToggle(p, text, def, cb)
    local st = def or false; toggleStates[text] = st
    local fr = Instance.new("Frame"); fr.Size = UDim2.new(1, -4, 0, 32)
    fr.BackgroundColor3 = Color3.fromRGB(28, 28, 42); fr.BorderSizePixel = 0; fr.Parent = p
    Instance.new("UICorner", fr).CornerRadius = UDim.new(0, 6)
    local lb = Instance.new("TextLabel", fr); lb.Text = text
    lb.Size = UDim2.new(1, -60, 1, 0); lb.Position = UDim2.new(0, 10, 0, 0)
    lb.BackgroundTransparency = 1; lb.TextColor3 = Color3.fromRGB(210, 210, 225)
    lb.Font = Enum.Font.Gotham; lb.TextSize = 13; lb.TextXAlignment = Enum.TextXAlignment.Left
    local tr = Instance.new("Frame", fr); tr.Size = UDim2.new(0, 42, 0, 22)
    ...
    local kn = Instance.new("Frame", tr); kn.Size = UDim2.new(0, 16, 0, 16)
    ...
    local bt = Instance.new("TextButton", fr); bt.Size = UDim2.new(1, 0, 1, 0)
    bt.BackgroundTransparency = 1; bt.Text = ""
    bt.MouseButton1Click:Connect(function()
        st = not st; toggleStates[text] = st
        TweenService:Create(tr, TweenInfo.new(0.2), {BackgroundColor3 = st and Color3.fromRGB(90, 45, 210) or Color3.fromRGB(55, 55, 75)}):Play()
        TweenService:Create(kn, TweenInfo.new(0.2, Enum.EasingStyle.Back), {Position = st and UDim2.new(1, -19, 0.5, -8) or UDim2.new(0, 3, 0.5, -8)}):Play()
        if cb then pcall(cb, st) end end) end

local function createSlider(p, text, mn, mx, def, cb)
    sliderValues[text] = def
    local fr = Instance.new("Frame"); fr.Size = UDim2.new(1, -4, 0, 48)
    fr.BackgroundColor3 = Color3.fromRGB(28, 28, 42); fr.BorderSizePixel = 0; fr.Parent = p
    Instance.new("UICorner", fr).CornerRadius = UDim.new(0, 6)
    local lb = Instance.new("TextLabel", fr); lb.Text = text .. ": " .. def
    lb.Size = UDim2.new(1, -10, 0, 20); lb.Position = UDim2.new(0, 10, 0, 2)
    lb.BackgroundTransparency = 1; lb.TextColor3 = Color3.fromRGB(210, 210, 225)
    lb.Font = Enum.Font.Gotham; lb.TextSize = 12; lb.TextXAlignment = Enum.TextXAlignment.Left
    local bg = Instance.new("Frame", fr); bg.Size = UDim2.new(1, -20, 0, 10)
    bg.Position = UDim2.new(0, 10, 0, 28); bg.BackgroundColor3 = Color3.fromRGB(45, 45, 65)
    ...
    fl.BackgroundColor3 = Color3.fromRGB(110, 60, 255); fl.BorderSizePixel = 0
    Instance.new("UICorner", fl).CornerRadius = UDim.new(1, 0)
    local ib = Instance.new("TextButton", bg); ib.Size = UDim2.new(1, 0, 1, 10)
    ib.Position = UDim2.new(0, 0, 0, -5); ib.BackgroundTransparency = 1; ib.Text = ""
    local sd = false
    local function upd(i)
        local r = math.clamp((i.Position.X - bg.AbsolutePosition.X) / bg.AbsoluteSize.X, 0, 1)
        local v = math.floor(mn + (mx - mn) * r); sliderValues[text] = v
        fl.Size = UDim2.new(r, 0, 1, 0); lb.Text = text .. ": " .. v
        if cb then pcall(cb, v) end end
    ib.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then sd = true; upd(i) end end)
    ib.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then sd = false end end)
...
local function createButton(p, text, cb)
    local bt = Instance.new("TextButton"); bt.Size = UDim2.new(1, -4, 0, 32)
    bt.BackgroundColor3 = Color3.fromRGB(28, 28, 42); bt.BorderSizePixel = 0
    bt.Text = text; bt.TextColor3 = Color3.fromRGB(210, 210, 225)
    bt.Font = Enum.Font.Gotham; bt.TextSize = 13; bt.Parent = p
    Instance.new("UICorner", bt).CornerRadius = UDim.new(0, 6)
    bt.MouseButton1Click:Connect(function()
        TweenService:Create(bt, TweenInfo.new(0.08), {BackgroundColor3 = Color3.fromRGB(90, 45, 210)}):Play()
        task.delay(0.15, function() TweenService:Create(bt, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(28, 28, 42)}):Play() end)
        if cb then pcall(cb) end end)
    return bt
end

local function notify(text, duration)
    local n = Instance.new("TextLabel"); n.Size = UDim2.new(0, 320, 0, 42)
    n.Position = UDim2.new(0.5, -160, 1, 10); n.BackgroundColor3 = Color3.fromRGB(28, 28, 42)
    n.TextColor3 = Color3.fromRGB(190, 150, 255); n.Font = Enum.Font.GothamBold
    n.TextSize = 14; n.Text = text; n.Parent = ScreenGui
    Instance.new("UICorner", n).CornerRadius = UDim.new(0, 8)
    Instance.new("UIStroke", n).Color = Color3.fromRGB(110, 60, 255)
    TweenService:Create(n, TweenInfo.new(0.5, Enum.EasingStyle.Back), {Position = UDim2.new(0.5, -160, 1, -55)}):Play()
    task.delay(duration or 3, function()
        TweenService:Create(n, TweenInfo.new(0.4), {Position = UDim2.new(0.5, -160, 1, 50)}):Play()
        task.delay(0.5, function() n:Destroy() end) end)
end

-- =============================================
--  GOD MODE (FIXED - ANTI MATI)
-- =============================================
...
local function RemoveTracer(player)
    if Tracers[player] then
        pcall(Tracers[player].Remove, Tracers[player])
        Tracers[player] = nil
    end
end
...
local espHolder = Instance.new("Folder", ScreenGui); espHolder.Name = "ESP"
local espCache = {}

local function makeESP(player)
    if player == LocalPlayer then return end
    local folder = Instance.new("Folder", espHolder); folder.Name = player.Name
    ...
    espCache[player] = {folder = folder, bb = bb, nameL = nameL, healthL = healthL, distL = distL, hl = hl}
end
...
RunService.RenderStepped:Connect(function()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        if not espCache[player] then makeESP(player) end
        local e = espCache[player]
        if not e then continue end
        
        local show = toggleStates["Player ESP"]
        local char = player.Character
        
        -- Tracer
        if toggleStates["Tracer"] and show and char and char:FindFirstChild("HumanoidRootPart") then
        ...
        if show and char and char:FindFirstChild("HumanoidRootPart") then
            ...
            if toggleStates["ESP Health"] then
                ...
                if hum then
                    ...
                    e.healthL.Text = hp .. "/" .. mhp
                    e.healthL.Visible = true
                    e.healthL.TextColor3 = Color3.fromRGB(
                        math.floor(255 * (1 - pct * 0.3)),
                        math.floor(255 * (0.6 + pct * 0.4)),
                        math.floor(100 * pct + 50)
                    )
            ...
            if toggleStates["ESP Distance"] then
                ...
                if myC and myC:FindFirstChild("HumanoidRootPart") then
                    ...
                    e.distL.TextColor3 = Color3.fromRGB(255, 255, 0)
                end
            else
...
local function refreshPlayers()
    ...
    plCount.Text = "  Players: " .. math.max(#Players:GetPlayers() - 1, 0) .. " / " .. Players.MaxPlayers
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer then
            createButton(plBox, "→ " .. p.DisplayName .. " (@" .. p.Name .. ")", function()
                local char = LocalPlayer.Character
...
refreshPlayers()
Players.PlayerAdded:Connect(function(p) task.wait(1); refreshPlayers(); notify("➕ " .. p.DisplayName .. " joined!", 2) end)
Players.PlayerRemoving:Connect(function(p) refreshPlayers(); notify("➖ " .. p.DisplayName .. " left!", 2) end)
...
notify("🔥 Lynzka Hub v3.0 Loaded! | Press INSERT", 4)
